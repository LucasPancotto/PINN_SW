...
class PhysicsInformedNN:
    ...
    def __init__(self,
        ...
        self.adaptive = False
        if activation=='tanh':
            self.act_fn = keras.activations.tanh # note that act_fn stands for activation function
            self.kinit  = 'glorot_normal'        # kinit stands for weighy initialization
    ...
    def train(self,
        ...
        for ep in range(ep0, ep0+epochs):
            for ba in range(batches):
                ...
                # Get targets
                yloc    = X_batch[0][2]
                params_fit_mean = np.array([0.47095114, -0.1761286, -0.08489371, -0.02802178])
                target_mean = np.float32(np.polyval(params_fit_mean, yloc))
                params_fit_std = np.array([0.3482332, 0.05750806, -0.02944719, 1.00056959])
                target_std = np.float32(hyperbolic_tan(yloc, *params_fit_std))
                ...
                target_mean = tf.constant(target_mean, dtype='float32')
                target_std  = tf.constant(target_std, dtype='float32')
    ...
    def training_step(self, X_batch, Y_batch,
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # Statistics part (esto esta mal)
            # tf.print(target_mean, target_std)
            # aux1 = tf.square(tf.reduce_mean(Y_batch[:,0]) - target_mean)
            # aux2 = tf.square(tf.math.reduce_std( Y_batch[:,0]) - target_std)
            # loss_data = loss_data + lambda_data*(aux1+aux2)
...
def get_mini_batch(X, Y, ld, lf, ba, batches, flag_idxs, random=True):
    ...
    idxs  = np.random.choice(fi, len(fi)//batches)
...
def hyperbolic_tan(x, x0, s, a, b):
    return np.tanh((x-x0)/s) * a + b
